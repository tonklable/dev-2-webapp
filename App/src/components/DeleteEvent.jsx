        ...
        <div>
            ...
            <div tabIndex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal flex justify-center items-center md:h-full">
                <div class="relative w-full h-full max-w-xl md:h-auto">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 pt-8 pb-1">
                        ...
                        <h1 class="text-2xl text-center dark:text-gray-300">Are you sure to delete {event.name} ?</h1>
                        <div class="flex justify-center mt-6 mb-4">
                            {/* <button class="px-6 py-2 text-xl text-center text-blue-100 bg-red-600 rounded" onClick={() => deleteDoc(doc(database,"events/"+event.id)).then(()=>{alert('event has been deleted')}).then(()=>refreshPage())}> Yes</button> */}
                            <button class="px-6 py-2 text-xl text-center text-blue-100 bg-red-600 rounded" onClick={() => deleteDoc(doc(database,"events/"+event.id)).then().then(()=>refreshPage())}> Yes</button>
                            <button class="px-6 py-2 text-xl text-center text-blue-100 bg-blue-600 rounded ml-4" onClick={() => {props.closeDeleteEvent(false); props.setOpenFullpage(true)}}> No </button>
                        </div>
                    </div>
                </div>
            </div>
